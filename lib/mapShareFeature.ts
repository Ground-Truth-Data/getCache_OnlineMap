 ...
 *  src/lib/mobile/utils/retreeverFile.ts (structurally identical — rapper
...
// rapper stays UI-only, no proprietary import).
...
export function featureToKML(feature: Feature): string {
    ...
    // "isRetreever" MUST match RETREEVER_STAMP in tinySchema.ts — rapper can't
    ...
    // consumes — rapper only ferries it). Same escape hatch as featureData.
    const contactsData =
...
// A <description> generated by Get Cache's KMZ exporter (kmzBalloon.ts) is
...
// RT_BALLOON_MARK in kmzBalloon.ts — duplicated here because rapper can't
// import from proprietary src/ (same arrangement as the isRetreever stamp).
const RT_BALLOON_MARK = "<!--rt-balloon-->";

// Exported for the balloon-guard unit tests (kmzExport.test.ts) — node has no
// DOMParser, so the guard is asserted here rather than through kmlToFeatures.
export function normaliseProps(p: Feature["properties"]): Record<string, unknown> {
    const props: Record<string, unknown> = { ...(p ?? {}) };
    if (
        typeof props.description === "string" &&
        props.description.includes(RT_BALLOON_MARK)
    ) {
