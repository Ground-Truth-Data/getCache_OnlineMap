...
function addHospitalLayers(map: mapboxgl.Map): void {
    ...
    const openHospitalPopup = (
        ...
        // import of ReTreever's rendererOf.ts: rapper is UI-only and must not
        // import from `$lib`.
        const PopupCtor = map
            .getCanvasContainer?.()
...
async function fetchHospitals(
    ...
    try {
        ...
        // the layer either draws or the catch below shouts. (rapper is open-core
...
export function initializeMap(
    ...
    map.on("load", async () => {
        map.resize();
        if (opts.showHospitalMarkers) {
            // Anchor comes from the APP, not from here: rapper is UI-only and
